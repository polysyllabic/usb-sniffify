...
void log_control_request(struct usb_ctrlrequest *ctrl) {
  PLOG_VERBOSE << "bRequestType: " << (int) ctrl->bRequestType << " (" <<
    ...
    case USB_TYPE_STANDARD:
      switch (ctrl->bRequest) {
        ...
        case USB_REQ_SET_FEATURE:
          ...
          break;
        default:
          PLOG_VERBOSE << "  unknown req = 0x" << std::hex << (int) ctrl->bRequest << std::dec;
          break;
      }
      break;
    case USB_TYPE_CLASS:
...
void log_event(struct usb_raw_event *event) {
  ...
  case USB_RAW_EVENT_CONNECT:
    PLOG_VERBOSE << "event: connect, length: " << event->length;
    break;
